...
class GatewayList:
    def __init__(self):
        g1 = Gateway(eth='00:00:00:00:00:01',ip_addr='192.168.1.254', mask=24)
        g2 = Gateway(eth='00:00:00:00:00:02',ip_addr='192.168.2.254', mask=24)
        self.list = [g1, g2]
    def get_all(self):
        return self.list
...
class QsysTest(SimpleSwitch13):
    ...
    def gw_send_arp(self, src_eth, src_ip, gw_eth, gw_ip, opcode, dp):
        ...
        if opcode == arp.ARP_REQUEST:#ARP Request
            ...
            a = ARP(1, 0x0800, 6, 4, opcode, src_eth, src_ip, target_eth, target_ip)
            p = packet.Packet()
            p.add_protocol(e)
            p.add_protocol(a)
            p.serialize()
